...
static void drawPrompt(string mainMessage, string secondMessage, bool start = 0);
static void drawScoreAndLives(string);

enum GameController::GameControllerState : int {
    welcome, init, makemove, animate, contgame, finishedlevel, gameover, cleanup, quit, prompt, not_applicable, prompt2
};
...
bool GameController::passesThruWhenSingleStepping(int key) const
{
	static set<int> passThruKeys = {
		...
		KEY_PRESS_UP, KEY_PRESS_DOWN, KEY_PRESS_ENTER, 'n','m'
	};
...
void GameController::keyboardEvent(unsigned char key, int /* x */, int /* y */)
{
	switch (key)
	{
		...
		case 'n':			m_lastKeyHit = 'n';				break;		//
		case 'm':			m_lastKeyHit = 'm';				break;		//
...
void GameController::doSomething()
{
	switch (m_gameState)
	{
		...
		case prompt:
			...
			break;
		case prompt2:
			drawPrompt(m_mainMessage, m_secondMessage, true);
			{
				int key;
				if (getKeyIfAny(key) && key == '\r')
					setGameState(m_nextStateAfterPrompt);
...
static void drawPrompt(string mainMessage, string secondMessage, bool start)
{
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
	glColor3f(1.0, 1.0, 1.0);
	glLoadIdentity();
	outputStrokeCentered(1, -5, mainMessage.c_str());
	//if (start) outputStrokeCentered(0, -5, "Move: WASD / Arrows    Shoot: Space");
...
static void drawScoreAndLives(string gameStatText)
{
	...
	outputStrokeCentered(3.3, SCORE_Z, "Move: WASD / Arrow     Shoot : Space     Restart : ESC     Quit : q");
